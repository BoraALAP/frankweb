...
const PRODUCT_QUERY = gql`
  query PRODUCT_QUERY($productid: ID) {
    transomsConnection(where: { Id: $productid }) {
      aggregate {
        count
      }
      edges {
...
const TemplateTransom = ({ match }) => {
  ...
  useEffect(() => {
    if (!loading && data.transomsConnection !== undefined) {
      setInfo({ ...data.transomsConnection.edges[0].node });
    }
  }, [loading]);
...
export default TemplateTransom;
